...
from etls.etl_helpers import string_to_blank_save_date, string_to_blank_save_numeric, try_str_to_float
...
class PatpopScrub:
    ...
    def _get_notes(self):
        ...
        if self.string_intention != "blank":
            if self.format == "date":
                return pd.Series([x if x in self.vals_to_notes else np.nan for x in self.old_col], name=self.notes_col_name)
            elif self.format == "numeric":
                return pd.Series([x if type(try_str_to_float(x)) == str else np.nan for x in self.old_col])
            else:
